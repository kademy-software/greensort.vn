$(document).ready(function () {
    ...
    fetch(coordinatesURL).then(function (response) {
      if (response.ok) {
        response.json().then(function (data) {
          ...
          lay_kq_chi_so_khong_khi
      (Latitude, Longitude, locDetails);
        });
      }
    });
  }
  ...
  function lay_kq_chi_so_khong_khi(lat, lng, locDetails) {
  ...
  function displayResults(aqi, locDetails) {
    ...
    displayAQI.text("Chất lượng không khí là " + aqi + " tại " + locDetails);
    ...
    if (aqi < 50) { // if air quality = good
      ...
      resultMessage.text('Chất lượng không khí tốt, không ảnh hưởng đến sức khoẻ');
      campContainer.removeClass('hide');
    } 
      else if (aqi > 50 && aqi < 100) { // if air quality = moderate
      resourcesContainer.addClass('hide');
      // Add warning message styling 
      resultMessage.addClass('ui floating positive message');
      resultMessage.text('Chất lượng không khí chấp nhận được. Tuy nhiên, đối với những người nhạy cảm (người già, trẻ em, người mắc các bệnh hô hấp, tim mạch..) có thể chịu những tác động nhất định đến sức khoẻ.');
      campContainer.removeClass('hide');
    }   else if (aqi > 101 && aqi < 150) { 
      resourcesContainer.addClass('hide');
      // Add warning message styling 
      resultMessage.addClass('ui floating warning message');
      resultMessage.text('Những người nhạy cảm gặp phải các vấn đề về sức khoẻ, người bình thường ít ảnh hưởng.');
      campContainer.removeClass('hide');
    } 
      ...
      resultMessage.text('Những người bình thường bắt đầu có các ảnh hưởng tới nhóm sức khoẻ, nhóm người nhạy cảm có vấn đề nghiêm trọng hơn.');
      campContainer.removeClass('hide');
    } 
